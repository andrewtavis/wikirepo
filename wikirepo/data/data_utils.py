...
def _get_dir_fxns_dict(dir_name=None):
    ...
    for mod in target_modules:
        ...
        name_fxn_list = [[fxn[0], getattr(script, fxn[0])] for fxn in inspect.getmembers(script, inspect.isfunction)]
...
def gen_base_df(locations=None,
    ...
    if type(locations) == list:
        assert depth == 0, """The user has provided locations with depth 0, but the 'depth' argument is not 0. 
        ...
        if depth != None:
            ...
            assert depth_check == depth, "The given depth and the derived depth of the LocationsDict do not match. Please check the geographic depth you want to analyze."
    ...
    if type(locations) == lctn_utils.LocationsDict or type(locations) == dict:
        base_df[lctn_utils.depth_to_col_name(depth=current_depth)] = \
            [list(lctn_utils.iter_key_items(node=locations, kv=q))[0]['lbl'] for q in current_depth_qids]
    ...
    while current_depth < depth:
        ...
        for q in [qid for qid in current_depth_qids if qid != np.nan]:
            ...
            key_subs = list(lctn_utils.iter_key_items(node=key_items, kv='sub_lctns'))[0]
            key_sub_qids = list(key_subs.keys())
            if key_sub_qids == []:
                # Assign to locations that don't have sub_lctns for pd.explode
                key_sub_qids = np.nan

            base_df.at[base_df[base_df[current_qid_col] == q].index[0], \
                assign_qid_col] = key_sub_qids
            ...
            if type(key_sub_qids) == list:
                for sub_q in base_df.loc[base_df[current_qid_col] == q, assign_qid_col]:
                    ...
                    base_df.loc[base_df[base_df[assign_qid_col] == sub_q].index[0], assign_lbl_col] = \
                            list(lctn_utils.iter_key_items(node=locations, kv=sub_q))[0]['lbl']
        ...
        if type(locations) == lctn_utils.LocationsDict or type(locations) == dict:
            ...
            for q in base_df[final_sub_lctn_qid_col]:
                if q != 'nan': # is str because of astype(str)
                    key_items = list(lctn_utils.iter_key_items(node=locations, kv=q))[0]
                    key_vts = list(lctn_utils.iter_key_items(node=key_items, kv='valid_timespan'))[0]

                    base_df.at[base_df[base_df[final_sub_lctn_qid_col] == q].index[0], \
                        time_col] = key_vts
        ...
        for q in df[assignment_col].unique():
            if type(q) == str: # is a valid location
                if len(props[q].keys()) == 1:
                    ...
                    assignment_times = sorted([k for k in props[q].keys() if k != 'no date'])[::-1]
                ...
                if type(props[q][most_recent_t]) == list:
                    ...
                    if span:
                        ...
                        df.loc[df.loc[df[assignment_col] == q].index, col_name] \
                            = ', '.join([str(i)  for i in props[q][most_recent_t]]) + f" ({most_recent_t})"
                
                else:
                    if span:
                        # We don't want the time for most recent span values
                        df.loc[df.loc[df[assignment_col] == q].index, col_name] \
                            = props[q][most_recent_t]
            ...
            if type(q) == str: # is a valid location
                ...
                if type(props[q]) == list:
                    ...
                    df.loc[indexes_to_assign, col_name] \
                            = [', '.join([str(i) for i in props[q]])] * len(indexes_to_assign)
                
                else:
                    df.loc[indexes_to_assign, col_name] \
                            = [props[q]] * len(indexes_to_assign)
            ...
            if type(q) == str: # is a valid location
                ...
                for k in props[q][most_recent_t].keys():
                    ...
                    if span == True and sub_pid == bool:
                        ...
                        df.loc[df[(df[assignment_col] == q)].index[0], sub_col] \
                                = props[q][most_recent_t][k]
...
def query_wd_prop(dir_name=None,
    ...
    if type(locations) == list:
        qids = [lctn_utils.lctn_lbl_to_qid(lctn) if not wd_utils.is_wd_id(lctn) else lctn for lctn in locations]
    ...
    if col_prefix == None:
        t_to_p_dict = wd_utils.t_to_prop_val_dict(dir_name=dir_name,
                                                  ents_dict=ents_dict, 
                                                  qids=qids, 
                                                  pid=pid,
                                                  sub_pid=sub_pid,
        ...
        if interval is not None:
            ...
            df = gen_base_and_assign_to_column(locations=locations,
                                               ...
                                               props=t_to_p_dict, 
                                               assign='most_recent',
                                               span=span) # to remove the time from span props

    else:
        t_to_p_dict = wd_utils.t_to_prop_val_dict_dict(dir_name=dir_name,
                                                       ents_dict=ents_dict, 
                                                       qids=qids, 
                                                       pid=pid,
                                                       sub_pid=sub_pid,
        ...
        if interval is not None:
            ...
            df = gen_base_and_assign_to_cols(locations=locations,
                                             ...
                                             props=t_to_p_dict, 
                                             assign='most_recent',
                                             span=span) # to remove the time from boolean span props
...
def query_repo_dir(dir_name=None, 
    ...
    modules_to_query = [arg for arg in list(local_args['kwargs'].keys()) \
    ...
    for mod in tqdm(modules_to_query, desc=dir_name.capitalize(), disable=disable):
        ...
        query_fxn = [f for f in list(module_fxns.keys()) if f[:len('query_')] == 'query_'][0] # there can only be one per module
...
def interp_by_subset(df=None, 
    ...
    if 'method' in kwargs.keys() and 'limit_direction' in kwargs.keys() \
        and kwargs['method'] == 'pad' and kwargs['limit_direction'] == 'both':
        for lctn in unique_lctns:
            df_subset = df[df[lctn_utils.depth_to_col_name(depth)] == lctn].copy()
            unique_val = [val for val in df_subset[col_name].unique() if not (type(val) != str and np.isnan(val))][0]
...
def sum_df_prop_vals(df=None, 
    ...
    if time_col:
        for t in df_new[time_col].unique():
            if subtract:
                df_new.loc[df_new[(df_new[lctn_col] == target_lctn) & (df_new[time_col] == t)].index, prop_col] = \
                    df_new.loc[df_new[(df_new[lctn_col] == target_lctn) & (df_new[time_col] == t)].index, prop_col].values[0] - \
                        df_new.loc[df_new[(df_new[lctn_col] == vals_lctn) & (df_new[time_col] == t)].index, prop_col].values[0]
        ...
        if subtract:
            df_new.loc[df_new[df_new[lctn_col] == target_lctn].index, prop_col] = \
                df_new.loc[df_new[df_new[lctn_col] == target_lctn].index, prop_col].values[0] - \
                    df_new.loc[df_new[df_new[lctn_col] == vals_lctn].index, prop_col].values[0]
...
def split_col_val_dates(df=None, 
                        col=None):
    """
    Adds or subtracts values in a dataframe for given locations
    ...
    df_new[f"{col}_date"] = [val.split(' (')[1] if val != np.nan else np.nan for val in df_new[col]]
    df_new[col] = [val.split(' (')[0] if val != np.nan else np.nan for val in df_new[col]]

    df_new[f"{col}_date"] = [d.replace(')', '') if d != np.nan else np.nan for d in df_new[f"{col}_date"]]
...
def count_df_prop_vals(df=None, 
                       col=None, 
                       percent=False):
    """
