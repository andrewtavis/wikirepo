...
def test_lctn_to_qid_dict():
    assert isinstance(lctn_utils.lctn_to_qid_dict(), dict)


def test_qid_to_lctn_dict():
    assert isinstance(lctn_utils.qid_to_lctn_dict(), dict)


def test_incl_lctn_lbls():
    assert len(lctn_utils.incl_lctn_lbls("world")) == 1
    assert len(lctn_utils.incl_lctn_lbls("continent")) == 7
    assert isinstance(lctn_utils.incl_lctn_lbls("country"), list)
    assert isinstance(lctn_utils.incl_lctn_lbls("region"), list)


def test_incl_lctn_ids():
    assert isinstance(lctn_utils.incl_lctn_ids(), list)


def test_qid_to_lctn_lbl(qid):
    assert isinstance(lctn_utils.qid_to_lctn_lbl(qid), str)


def test_merge_lctn_dicts(lctns_dict):
    assert isinstance(
        lctn_utils.merge_lctn_dicts(ld1=lctns_dict, ld2=lctns_dict),
...
def test_LocationsDict(ents_dict, lctns_dict):
    ...
    assert isinstance(lctns_dict.key_lbls_at_depth(ents_dict=ents_dict, depth=0), list)
